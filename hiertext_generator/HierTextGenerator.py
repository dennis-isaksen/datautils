...
class HierTextGenerator:
    def __init__(self, word_list, fill_words, font_paths, font_sizes, backgrounds=None,
    ...
    def generate_paragraph(self, max_words=5):
        """
        Generate a paragraph-like structure by mixing words from the word_list with fill words.
        """
        paragraph = []
        for _ in range(max_words):
            if random.random() < 0.5:  # 50% chance to pick a word from word_list
                paragraph.append(random.choice(self.word_list))
            else:  # 50% chance to pick a fill word
                paragraph.append(random.choice(self.fill_words))
    ...
    def apply_transformations(self, image, rotation=None, blur_radius=None, background=None):
        ...
        """
        # 1. Rotate the image (expanding canvas)
        ...
        # 3. Add random background if backgrounds are provided
        if background:
    ...
    # Function to wrap data in HierText-like structure
    ...
    def from_config(cls, config_path, debug=False):
        ...
        with open(config_path, "rb") as f:
            cfg = tomllib.load(f)
        # Aggregate word lists, etc.
        all_word_list = []
        all_fill_words = []
        for language in cfg["languages"]:
            all_word_list.extend(language["word_list"])
            all_fill_words.extend(language["fill_words"])
        return cls(
            all_word_list,
            all_fill_words,
            cfg["fonts"],
